...
import { SolrCollectionSchemaAddFieldParams } from './generated';
...
export class SolrCollectionSchemaFields {
     ...
     * @protected
     */
    static defaultFieldProperties: Omit<SolrCollectionSchemaAddFieldParams, 'name' | 'type'> = {
        /**
     ...
     */
    async list() {
        const { fields } = await this.schema.request<SolrCollectionSchemaGetRequest, object, SolrSchemaFieldsResponse>({
            method: 'get',
            endpoint: '/fields',
        });
        return fields;
     ...
     * @public
     */
    async delete(fields: MaybeCollection<Pick<SolrCollectionSchemaAddFieldParams, 'name'>>) {
        await this.schema.request<SolrCollectionSchemaPostRequest>({
            method: 'post',
            data: {
    ...
    async update(fields: MaybeCollection<SolrCollectionSchemaAddFieldParams>) {
        ...
        const prevFields = prevUnfilteredFields.filter((field) => !field.name.startsWith('_'));
        const prevFieldsByName = keyBy(prevFields, 'name');
        const prevFieldNames = Object.keys(prevFieldsByName);

        const fieldNamesToReplace = intersection(fieldNames, prevFieldNames).filter((filedName) => {
        ...
        const fieldNamesToDelete = difference(prevFieldNames, fieldNames).filter(name => name !== 'id');
