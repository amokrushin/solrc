...
export interface SolrRequest {
    request<D = object, P = object, R = unknown>(params: SolrRequestParams<D, P>): Promise<R>;
}
...
export class SolrClient {
     ...
     */
    async request<D = object, P = object, R = SolrResponse>(params: SolrRequestParams<D, P>): Promise<R> {
        const {
            method = 'get',
            endpoint = '',
            data = {},
        ...
        const requestParams = new URLSearchParams(preprocessParams({
            ...restParams.params,
            omitHeader: false,
        }));
        ...
        if (checkResponse) {
            if (response?.responseHeader?.status !== 0) {
                console.dir(params, { colors: true, depth: 2 });
                console.dir(response, { colors: true, depth: 4 });
